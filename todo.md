...
* [When a P-Switch is used, Rolling Hill coins turn into Brick Blocks only visually -- they still behave like coins when touched](https://cdn.discordapp.com/attachments/601848765183295537/1498501521556504680/coin.mp4?ex=69f20c8b&is=69f0bb0b&hm=0394a58f9948efbdc7a7ef8b0c1d50c7c71b6f72eb9d511e7ab2ea96b6ab3906&)
* From Gimzie: "In NSMBWii, the animations for the four-way cannonball launcher are done entirely in code, including the shoot animation. The shoot animation is created by scaling the X and Y for each gun end bone. However, you will notice that some of the cannons appear to stretch much wider than intended. This is due to an error regarding how the effect was applied. The default orientation of the model has two cannons facing up/down, and the other two along the sides. The scale for the ones on the sides should be flipped such that the Y scale is applied to the X axis and vice versa. Nintendo did not do this, and so this effect is broken in-game."
* From Ryguy: ["Take a "Climbing Koopa - Horizontal" sprite and make it spawn behind the chainlink fence. When it crosses over the fence, it "flips" over wrong."](https://cdn.discordapp.com/attachments/1204009140255596554/1510772544519671998/2026-05-31_17-30-23.mp4?ex=6a1e0813&is=6a1cb693&hm=2a95dd2cf557124f8a3862402dc65aced60a7bc6a9b18bc4f1324bd451e3e95a&)
* From Mandy: ["If you ground-pound a Spine Coaster that is only one piece long (so just the head), it'll crash the game because I think it tries to apply the wobbling platform effect to the next platform behind it... which doesn't exist. This happens at 0x8085a770"](https://cdn.discordapp.com/attachments/996508429311033455/1511107425254506628/image.png?ex=6a1f3ff5&is=6a1dee75&hm=8dea7a6d55d080b9854e1915f19066c323c563b531ab82912331731d9e8322fc&)
* From CLF: "FUN_808e3400 is the one that sets the animation for daWmPeach_c. Since the function is called with param_5 - 1, it means passing 0 will give an invalid index and crash the game. Must be a coincidence that Peach is not spawned and so the code never runs. Peach doesn't appear before 8-Airship. Anyway the bug is in daWmBossKameck_c."
* From B1 Gaming: "The midway flag only plays its slower underwater animation if you placed it underwater after you added water to your zone. If you add the midway before adding the water, it plays its normal wait animation."
* From Ryguy: "The dEn_c player collision callback doesn't check if the collider has the star bitflag set in the bitfield used to determine what player attacks it can collide with, meaning that players with a star can always hurt enemies using the default dEn_c collision callback. Every enemy that shouldn't die to stars just overrides the star collision function to get around this, so the fix shouldn't actually break anything (I haven't seen any cases of an actor not setting the star flag when it's obvious they should be killed by a star). NSMBU fixes this so I'm pretty sure it's a bug."
